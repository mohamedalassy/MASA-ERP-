...
class AiSalesAdvancedController extends Controller
{
    public function territories()
    {
        ...
        $territoryRows = $territories
            ->map(function (AiSalesTerritory $territory) use ($companies) {
                $matchedCompanies = $companies
                    ->filter(
                        fn (AiSalesCompany $company) =>
                            $this->companyMatchesTerritory(
                                $company,
                                $territory
                            )
                    )
        ...
        $allOpportunities = $companies
            ->flatMap(fn ($company) => $company->opportunities);

        $allLeads = $companies
            ->flatMap(fn ($company) => $company->leads);

        $allSignals = $companies
            ->flatMap(fn ($company) => $company->signals);
        ...
        $scoredCompanies = $companies
            ->filter(fn ($company) => $company->latestScore);
        ...
        return response()->json([
            ...
            'summary' => [
                ...
                'average_ai_score' => $scoredCompanies->count()
                    ? round(
                        $scoredCompanies->avg(
                            fn ($company) =>
                                (float) $company->latestScore->total_score
                        ),
                        1
                    )
                    : 0,
            ...
            'unassigned' => [
                ...
                'leads' => $unassignedCompanies
                    ->flatMap(fn ($company) => $company->leads)
                    ->count(),
                'opportunities' => $unassignedCompanies
                    ->flatMap(fn ($company) => $company->opportunities)
                    ->count(),
                'signals' => $unassignedCompanies
                    ->flatMap(fn ($company) => $company->signals)
                    ->count(),
                'cities' => $this->buildCityRows(
                    $unassignedCompanies
                ),
            ],
    ...
    private function companyMatchesTerritory(
        ...
        $cities = collect($territory->cities ?? [])
            ->filter()
            ->values();
        ...
        $industries = collect($territory->industries ?? [])
            ->filter()
            ->values();
        ...
        /*
         * A territory with no geographic/industry criteria
         * would otherwise match every company.
         */
        return (bool) (
            $territory->country ||
            $territory->region ||
    ...
    private function sameLocation(
        ...
        return mb_strtolower(trim($left)) ===
    ...
    private function buildCityRows(
        Collection $companies
    ): array {
        return $companies
            ->filter(
            ...
            ->map(function (Collection $cityCompanies, string $city) {
                ...
                $opportunities = $cityCompanies
                    ->flatMap(
                        fn ($company) =>
                            $company->opportunities
                    );

                $signals = $cityCompanies
                    ->flatMap(fn ($company) => $company->signals);
