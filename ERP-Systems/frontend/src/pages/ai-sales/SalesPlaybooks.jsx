...
export default function SalesPlaybooks({onNavigate,activeView="ai-sales-playbooks"}) {
  ...
  return <Shell activeView={activeView} onNavigate={onNavigate} title="خطط المبيعات"
    subtitle="مسارات عمل قابلة لإعادة الاستخدام محفوظة داخل المبيعات الذكية.">
    {error&&<div className="ai-error">{error}</div>}
    <div className="workspace-2">
      <Panel title="خطط المبيعات" action={<Btn secondary onClick={load}><RefreshCw size={14}/> Refresh</Btn>}>
        <div className="playbook-grid">{rows.length?rows.map((x,i)=><article key={x.id}>
          ...
          <small>{(x.steps||[]).length} steps • {x.trigger_type||"manual"} • {x.is_active?"نشط":"غير نشط"}</small>
        </article>):<p>No playbooks yet.</p>}</div>
      </Panel>
      <Panel title="خطة جديدة"><form className="pro-form" onSubmit={save}>
        ...
        <label>الوصف</label><textarea value={form.description} onChange={e=>setForm({...form,description:e.target.value})}/>
